...
constexpr const auto VECTOR_SIZE = 1000000;
...
inline void parallelGenerate() noexcept
{
    ...
    Parallel::ForeachRestricted(vector.begin(), vector.end(), GenerateFunction, 3);
}
...
int main()
{
    ...
    
    {
        ...
        executionGenerate();
        const auto end = std::chrono::steady_clock::now();
        std::cout << "Execution generate      : " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() << " ms" << std::endl;    
    }
